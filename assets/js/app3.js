...
function onBlogAdd(eve) {
    ...
    makeApiCall(POST_URL, "POST", blogObj)
        .then(data => {
            ...
            snackBar(`blog crated successfully!`, "success")
        })
        .catch(err => {
            snackBar('something went wrong while creating new blogs!!!', 'error')
        })
}
...
function onUpdate() {
    ...
    makeApiCall(UPDATED_URL, "PATCH", UPDATED_OBJ)
        ...
        .catch(err => {
            cl(err);
            snackBar(`Something went wrong while updating BLOGs!`, "error");
        })
